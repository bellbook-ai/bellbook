...
def run_malformed_cases() -> tuple[int, list[str]]:
    ...
    for c in cases:
        limits = c.get("limits")
        max_bytes = limits["max_bytes"] if limits else None
        max_records = limits["max_records"] if limits else None
        status, _problem = bb.validate(c["input"], max_bytes=max_bytes, max_records=max_records)
        check(
